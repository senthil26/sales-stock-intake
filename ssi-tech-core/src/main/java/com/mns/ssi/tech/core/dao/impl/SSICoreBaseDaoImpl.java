...
public class SSICoreBaseDaoImpl implements SSICoreBaseDao {
    ...
    @Autowired
    public SSICoreBaseDaoImpl(EntityManager entityManager) {
        this.entityManager = entityManager;
    }

    /**
     * @param entity
     * @return this will return the saved model if persist is successful
     */
     ...
     * @return
     */
    public <T extends SSIBaseEntity, I> List<T> findByFields(final Class<T> entityClass, final String searchField,
                                                             final List<I> fieldValues, boolean isNegative) {
        List<T> queryResult = null;
        try {
            CriteriaBuilder criteriaBuilder = entityManager.getCriteriaBuilder();
            CriteriaQuery<T> criteriaQuery = criteriaBuilder.createQuery(entityClass);
            ...
            Predicate predicate = exp.in(fieldValues);
            if (isNegative) {
                criteriaQuery.where(predicate.not());
            } else {
                criteriaQuery.where(predicate);
